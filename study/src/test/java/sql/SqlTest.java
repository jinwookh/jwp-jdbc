...
public class SqlTest {
    ...
    @Test
    void hobby_설문결과() {
        List<HobbySurvey> hobbySurveys = jdbcTemplate.listQuery(hobbyQuery,
    ...
    void professional_coding_experience_설문결과() {
        ...
        List<CodingExperienceServey> hobbySurveys = jdbcTemplate.listQuery(codingExperienceQuery,
    ...
    void hobby_쿼리_시간_계산() {
        assertTimeout(Duration.ofMillis(100), () -> {
            jdbcTemplate.listQuery(hobbyQuery,
                    rs -> new HobbySurvey(rs.getString("Hobby"), rs.getFloat("ratio")));
        });
    }

    @Test
    void codingExperience_쿼리_시간_계산() {

        assertTimeout(Duration.ofMillis(100), () -> {
            jdbcTemplate.listQuery(codingExperienceQuery,
                    rs -> new CodingExperienceServey(rs.getString("job"), rs.getFloat("coding_experience")));
        });
    }
}
